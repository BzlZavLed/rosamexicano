...
if($user == 1 || $user == 2){
    if($fecha1 == "-" || $fecha2 == "-"){
        $cajaventas = "SELECT DISTINCT e.*,TRIM(f.vendedor) as vendedor,f.ie as ie,f.concepto as concepto,f.metodo as metodo, g.nombre as provname, (e.pUni * e.cant) - total  as descuento,e.hora as hora FROM ventadesg e INNER JOIN ventas f ON e.idventa = f.idventa INNER JOIN proveedores g ON e.proveedor = g.ident WHERE e.fecha ='".$fecha."'";
    }else{
