  ...
  if (reptip == 1) {
          ...
          var table = $("#reportesTable1").DataTable({
            ...
            buttons: [
              ...
              { extend: "pdfHtml5", footer: true },
              {
                ...
                customize: function (win) {
                  $(win.document.body).css("font-size", "10pt").append(head);
            ...
            footerCallback: function (row, data, start, end, display) {
              ...
              var formatted = $.fn.dataTable.render
                .number(",", ".", 0, "$")
                .display(total);
              $(api.column(8).footer()).html(formatted);
              // Total over all pages
              total = api
              ...
              var formatted = $.fn.dataTable.render
                .number(",", ".", 0, "$")
                .display(total);
              $(api.column(13).footer()).html(formatted);
            },
          });
    ...
    $.ajax({
      ...
      success: function (resp) {
        ...
        document.getElementById("qrCreator").style.display = "block";
    ...
    $.ajax({
      ...
      success: function (resp) {
        ...
        if (resp.resp[0].message == "nothing found") {
          swal(
            "Alerta",
            "No hay entradas registradas en el periodo seleccionado",
            "warning"
          );
        } else {
          var tr = document.getElementById("poliza");
      ...
      success: function (resp) {
        ...
        if (resp.resp[0].message == "nothing found") {
          ...
          for (i = 0; i < lt1; i++) {
            ...
            var ident =
              resp.resp[i]["ident"] != undefined ? resp.resp[i]["ident"] : ".";
            var nombre =
              resp.resp[i]["nombre"] != undefined
                ? resp.resp[i]["nombre"]
                : ".";
            var ventas =
              resp.resp[i]["ventas"] != undefined
                ? resp.resp[i]["ventas"]
                : ".";
            var pagos =
              resp.resp[i]["pagos"] != undefined ? resp.resp[i]["pagos"] : ".";
            var descuentos =
              resp.resp[i]["descuentos"] != undefined
                ? resp.resp[i]["descuentos"]
                : ".";
            descuentos = parseFloat(descuentos);
            descuentos = descuentos.toFixed(2);
            var button =
              '<button type="button" class="btn btn-primary" id="myBtn">Ventas</button><button type="button" class="btn btn-success" id="myBtnVentas">Pagos</button>';
    ...
    if (provid == null) {
      ...
      $("#marcaelegida").text(
        $("#proveedoresProductos option:selected").text()
      );
      $.ajax({
        ...
        success: function (resp) {
          ...
          if (resp.resp[0].message == "nothing found") {
            ...
            for (i = 0; i < resp.resp.length; i++) {
              ...
              var button =
                '<input type = "button" class = "btn btn-danger" id = "eliminarProd" value = "Eliminar">';
      ...
      success: function (resp) {
        ...
        if (resp.resp[0].message == "nothing found") {
          ...
          for (i = 0; i < resp.resp.length; i++) {
            ...
            var button =
              '<button class="btnico" id = "selectLine"><i class="fa fa-trash"></i></button>' +
              '<button class="btnico" id = "notificacionemail"><i class="fa fa-envelope"></i></button>';
            if (pagado == 3) {
              pagado = "Pagado";
            } else if (pagado == 2) {
              pagado = "Faltante";
    ...
    success: function (resp) {
      ...
      if (resp.resp[0].error == true) {
        ...
        for (i = 0; i < lt1; i++) {
          ...
          var idProd =
            resp.resp[i]["idProd"] != undefined ? resp.resp[i]["idProd"] : ".";
          var nombre =
            resp.resp[i]["nombre"] != undefined ? resp.resp[i]["nombre"] : ".";
          var fecha =
            resp.resp[i]["fecha"] != undefined ? resp.resp[i]["fecha"] : ".";
          var totalventa =
            resp.resp[i]["totalventa"] != undefined
              ? resp.resp[i]["totalventa"]
              : ".";
          var totalprod =
            resp.resp[i]["totalprod"] != undefined
              ? resp.resp[i]["totalprod"]
              : ".";
          var totaldesc =
            resp.resp[i]["totaldesc"] != undefined
              ? resp.resp[i]["totaldesc"]
              : ".";
  ...
  $(document).on("click", "#generatePdf", function () {
    ...
    SVGToImage({
      ...
      .then(function (base64image) {
        ...
        doc.save("codigoProducto" + $("#nomProd").val() + ".pdf");
      })
      .catch(function (err) {
        console.log(err);
      });
